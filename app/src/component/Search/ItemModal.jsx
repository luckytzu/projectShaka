...
import ImageZoomModal from './ImageZoomModal'; // Assure-toi de bien l'importer
    ...
    <>
      ...
      >
        ...
        >
          ...
          {item.imageUrl && (
            <div className={`w-full flex justify-center border-b p-4 relative group ${isFBI ? 'bg-black border-cyan-900' : 'bg-[#1a0f0a] border-[#4a2e1b]'}`}>
              <img 
                src={item.imageUrl} 
                alt={item.nom} 
              ...
              <button
                ...
                className={`absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 transition-opacity duration-200 cursor-zoom-in ${
                  isFBI ? 'text-cyan-300 font-mono' : 'text-amber-300 font-serif'
                }`}
              >
                <span className="text-2xl">🔍</span>
                <span className="text-sm bg-black/70 px-3 py-1 rounded border border-current">
                  {isFBI ? 'INSPECT_IMAGE' : 'Examiner en détail'}
                </span>
              </button>
